...
# ==========================================
st.set_page_config(
    page_title="田中式 土地家屋調査士一問一答｜法律特化AI講師とチャットで会話！その場で疑問をスピード解決", 
    page_icon="📖", 
    layout="centered"
)
...
def get_header_image_base64():
    path = "images/2_title.png" if os.path.exists("images/2_title.png") else "2_title.png"
    ...
    st.markdown(
        ...
        <div style="background-color: #f8fafc; padding: 25px; border-radius: 12px; text-align: center; border: 1px solid #cbd5e1; margin: 20px 0;">
            ...
            <p style="color: #475569; font-size: 1.05rem;">
                令和7年以外の過去問演習や、AIチャットのフル機能をご利用いただくには、<br>
...
if not st.session_state.get("user") and not st.session_state.get("trial_mode"):
    bg_pc_b64 = get_image_base64("images/2_background_PC.png") or get_image_base64("2_background_PC.png")
    bg_sp_b64 = get_image_base64("images/2_background_mobile.png") or get_image_base64("2_background_mobile.png")
...
def render_ai_teacher():
    if st.session_state.get("fast_mode", False):
        with st.sidebar:
            st.title("田中式 土地家屋調査士一問一答")
            st.markdown("### AIたなかっち2号先生")
            st.markdown("---")
        return

    image_map = {
        "normal": "images/2_teacher_normal.png",
        "thinking": "images/2_teacher_thinking.png",
        "happy": "images/2_teacher_happy.png",
        "sad": "images/2_teacher_sad.png",
    }
    current_state = st.session_state.get("teacher_state", "normal")
    img_path = image_map.get(current_state, image_map["normal"])
    
    with st.sidebar:
        title_path = "images/2_title.png" if os.path.exists("images/2_title.png") else "2_title.png"
...
def render_inline_chat(row):
    ...
    for msg in st.session_state.inline_messages:
        avatar_img = "images/2_teacher_normal.png" if msg["role"] == "assistant" else None
    ...
    if st.session_state.inline_waiting:
        st.session_state.inline_waiting = False
        with st.chat_message("assistant", avatar="images/2_teacher_normal.png"):
            status_text = "AIたなかっち2号先生が考えています..."
...
if menu == "年度別":
    ...
    if not df.empty and "問題番号" in df.columns:
        ...
        with ui_controls:
            ...
            with col_session:
                display_sessions = [s if ("令和7年" in s or is_premium) else f"{s} 🔒[有料会員限定]" for s in sessions]
                        ...
                        with ui_result:
                            ...
                            if st.session_state.y_user_ans == correct:
                                ...
                                with col_ok:
                                    st.success("正解です！")
                                with col_img:
                                    if not st.session_state.get("fast_mode", False) and os.path.exists("images/2_teacher_happy_o.png"):
                                        st.image("images/2_teacher_happy_o.png", width=45)
                            else:
                                col_err, col_img = st.columns([5, 1])
                                with col_err:
                                    st.error(f"不正解... （正解は {correct} です）")
                                with col_img:
                                    if not st.session_state.get("fast_mode", False) and os.path.exists("images/2_teacher_sad_x.png"):
                                        st.image("images/2_teacher_sad_x.png", width=45)
...
# ==========================================
    ...
    if not target_rows.empty:
        ...
        for i in range(total_batches):
            ...
            has_free_question = any("令和7年" in str(q) for q in batch_rows["問題番号"])
        ...
        if "🔒" in selected_batch_str:
            ...
            with st.spinner(f"第 {start_q + 1} 〜 {end_q} 問の音声をセット中..."):
                for _, row in current_batch_rows.iterrows():
                    ...
                    if not is_premium and "令和7年" not in q_num_val:
                        playlist.append({
...
# ==========================================
    ...
    if st.session_state.chat_count >= MAX_CHAT:
        ...
        if st.session_state.main_waiting:
            st.session_state.main_waiting = False
            with st.chat_message("assistant", avatar="images/2_teacher_normal.png"):
                with st.spinner("AIたなかっち2号先生が考えています..."):
